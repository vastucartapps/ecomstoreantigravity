...
class ShipStationProviderService extends AbstractFulfillmentProviderService {
  ...
  private async createShipment({
    ...
    const packageWeight = items.reduce((sum, item: any) => {
  ...
  async validateFulfillmentData(
    ...
    let { shipment_id } = (data as { shipment_id?: string }) || {}
    ...
    if (!shipment_id) {
      ...
      const shipment = await this.createShipment({
        ...
        carrier_service_code,
        from_address: {
          name: (context as any).from_location?.name,
          address: (context as any).from_location?.address,
        },
        to_address: (context as any).shipping_address,
        items: (context as any).items || [],
        currency_code: (context as any).currency_code,
      })
  ...
  async createFulfillment(
    ...
    items.map((item: any) => {
      const orderItem = (order as any)?.items?.find((i: any) => i.id === item.line_item_id)
      if (!orderItem) return
      orderItemsToFulfill.push({ ...orderItem, quantity: item.quantity })
    })
