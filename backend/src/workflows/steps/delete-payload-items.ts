...
export const deletePayloadItemsStep = createStep(
  ...
  async ({ where, collection }: StepInput, { container }) => {
    ...
    const prevData = await payloadModuleService.find(collection, { where })
    await payloadModuleService.delete(collection, { where })
    return new StepResponse({}, { prevData, collection })
  },
