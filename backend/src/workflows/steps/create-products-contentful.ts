...
export const createProductsContentfulStep = createStep(
  ...
  async (input: StepInput, { container }) => {
    const contentfulModuleService: ContentfulModuleService = container.resolve(CONTENTFUL_MODULE)
    ...
    try {
      ...
      return StepResponse.permanentFailure(`Product creation failed: ${e.message}`, products)
    }
  ...
  async (products, { container }) => {
    if (!products) return
    const contentfulModuleService: ContentfulModuleService = container.resolve(CONTENTFUL_MODULE)
    for (const product of products as EntryProps[]) {
