...
export const createPayloadItemsStep = createStep(
  ...
  async ({ items, collection }: StepInput, { container }) => {
    ...
    const createdItems = await Promise.all(
      items.map(async (item) => await payloadModuleService.create(collection, item))
    )

    return new StepResponse(
      { items: createdItems.map((item: any) => item.doc) },
      { ids: createdItems.map((item: any) => item.doc.id), collection }
    )
  },
  async (data, { container }) => {
    ...
    const payloadModuleService = container.resolve(PAYLOAD_MODULE)
    await payloadModuleService.delete(collection, { where: { id: { in: ids.join(",") } } })
  }
)
