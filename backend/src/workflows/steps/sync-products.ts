...
export const syncProductsStep = createStep(
  ...
  async ({ products }: SyncProductsStepInput, { container }) => {
    ...
    const newProducts = products.filter(
      (product) => !existingProducts.some((p: any) => p.objectID === product.id)
    )

    await algoliaModuleService.indexData(products as unknown as Record<string, unknown>[], "product")
  ...
  async (input, { container }) => {
    ...
    if (input.existingProducts) {
      await algoliaModuleService.indexData(input.existingProducts as any, "product")
    }
  }
)
