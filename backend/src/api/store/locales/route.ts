...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  const contentfulModuleService: ContentfulModuleService = req.scope.resolve(CONTENTFUL_MODULE)
  ...
  const formattedLocales = locales.items.map((locale: any) => ({
    name: locale.name,
    code: locale.code,
    is_default: locale.code === defaultLocaleCode,
  }))

  res.json({ locales: formattedLocales })
}
